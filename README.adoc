...
proxy.1.transform="[:sender :id :body]"
...
----
[:input-key                                               ; pass trough key
 [:input-key :output-key]                                 ; pass trough, rename key
...
For example, following rules

----
[:a                     ; pass-through :a
 [:b :bb]               ; rename :b to :bb
 [:c :cc 123 456]       ; set :cc to 456 if :c was 123, no hit
 [:c :cc 3 333]         ; set :cc to 333 if :c was 3
 [nil :new nil 77]]     ; set :new to 777
----
